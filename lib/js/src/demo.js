...
function trainMiniBatch(config, net, data) {
  ...
  if (errorData) {
    ...
    var weightErrors = linearClip(config[/* gradientClip */1], errorData$1[0]);
    updateWeights(net, weightErrors, config[/* alpha */4]);
...
function trainEpoch(config, net, data) {
  ...
  var match$1 = config[/* batchSize */5];
  var miniBatchSize = match$1 !== 0 ? config[/* batchSize */5] : dataLength;
  return $$Array.fold_left((function (iterDetails, _) {
                  var lastIndex = iterDetails[1];
                  var thisBatchSize = Pervasives.min(miniBatchSize, dataLength - lastIndex | 0);
                  var newError = iterDetails[0] + trainMiniBatch(config, net, $$Array.to_list($$Array.sub(finalDataArray, lastIndex, thisBatchSize)));
                  return /* tuple */[
                          newError,
                ...
                0.0,
                0
              ], Caml_array.caml_make_vect(1 + Caml_int32.div(dataLength, miniBatchSize) | 0, 0))[0];
}
